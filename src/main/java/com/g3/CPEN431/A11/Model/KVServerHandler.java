...
public class KVServerHandler implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (command <= 3 && command >= 1 && !requestMessage.hasClientAddress()) {
                ...
                if (command == Command.GET.getCode()) {
                    Collection<Node> backupNodes = getBackupNodesForKey(key);
                    if (!isPrimary(key) && !backupNodes.contains(nodesCircle.getNodeById(nodesCircle.getThisNodeId()))) {
    ...
    private Collection<Node> getBackupNodesForKey(ByteString key) {
        String sha256 = Hashing.sha256().hashBytes(key.toByteArray()).toString();
        int ringKey = nodesCircle.getCircleBucketFromHash(sha256.hashCode());
        return nodesCircle.findThreeImmediateSuccessors(ringKey);
    }
